namespace ShareTravelSystem.Services
{
    ...
    public class ReviewService : IReviewService
    {
        ...
        public async Task<EditReviewViewModel> GetReviewToEditAsync(int id, int offerId, string currentUserId)
        {
            ...
            var reviewAuthor = await this.db.Reviews.Where(r => r.Id == id && !r.IsDeleted).Select(x => x.AuthorId)
        ...
        {
            var review = await this.db.Reviews.Where(r => r.Id == reviewId && r.OfferId == offerId && !r.IsDeleted)
                .SingleOrDefaultAsync();
